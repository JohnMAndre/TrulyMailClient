Public Class EditContactK
    ...
    Private Sub KryptonCheckSet1_CheckedButtonChanged(sender As System.Object, e As System.EventArgs) Handles KryptonCheckSet1.CheckedButtonChanged
        Select Case KryptonCheckSet1.CheckedButton.Name
            Case chkbtnOther.Name
                pnlOther.BringToFront()
            Case chkbtnNotes.Name
                pnlNotes.BringToFront()
            Case chkbtnSettings.Name
                pnlSettings.BringToFront()
            Case chkbtnPicture.Name
                pnlPicture.BringToFront()
            Case chkbtnReminders.Name
                pnlReminder.BringToFront()
            Case chkbtnEmail.Name
    ...
    Public Sub New(ByVal userID As String)
        ...
        'AppSettings.Pallet.BasePaletteMode = ComponentFactory.Krypton.Toolkit.PaletteMode.SparklePurple
        'KryptonPalette1.BasePaletteMode = AppSettings.Pallet.BasePaletteMode
        ...
        Else
            ...
            If m_entry.ContactType = AddressBookEntryType.TrulyMail Then
                ...
                chkbtnEmail.Enabled = False
                pnlEmail.Enabled = False
                chkbtnReminders.Checked = True
                pnlReminder.BringToFront()
            Else
                pnlEmail.BringToFront()
                pbContctType.Image = My.Resources.email_32
    ...
    Public Property SMTPID As String
        Get
            Return CType(cboSMTPAccount.SelectedItem, SMTPProfile).ID
        End Get
        Set(value As String)
            If value = String.Empty Then
                cboSMTPAccount.SelectedIndex = 0 '-- use default account
            Else
                'For Each item As SMTPProfile In cboSMTPAccount.Items
                For Each item As Object In cboSMTPAccount.Items
                    If TypeOf item Is SMTPProfile AndAlso item.ID = value Then
                        cboSMTPAccount.SelectedItem = item
                        Exit For
                    End If
                Next
            End If
...
#Region " Control Changed Events "
    ...
    Private Sub txtEmailAddress_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        btnSave.Enabled = True
    End Sub

    Private Sub txtTags_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        btnSave.Enabled = True
    End Sub

    Private Sub cboAutoSendReturnReceipts_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        btnSave.Enabled = True
    End Sub

    Private Sub chkAutoRequestReturnReceipt_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        btnSave.Enabled = True
    End Sub
...
#End Region
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If m_boolEntryCreated Then
            '-- check if another contact has the same email address and warn user
            Dim entry As AddressBookEntry = ABook.GetContactByAddress(txtEmailAddress.Text)
            If entry IsNot Nothing Then
                If ShowMessageBox("A contact exists with this email address." & Environment.NewLine & Environment.NewLine & entry.NameWithAddress & Environment.NewLine & Environment.NewLine & "Are you sure you want to add this duplicate contact?", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.No Then
                    Exit Sub
                End If
            Else
        ...
        If rdoContactPictureNone.Checked Then
            ...
            If System.IO.File.Exists(GetContactPicturePath) Then
                DeleteLocalFile(GetContactPicturePath)
            End If
    ...
    Private Sub cboBuiltInPicture_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboBuiltInPicture.SelectedIndexChanged, cboBuiltInPicture.Click
        If rdoContactPictureBuiltIn.Checked Then
            SelectBuildInPicture()
        End If
    End Sub
    ...
    Private Sub EditContact_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Catch ex As Exception
            ...
            Log("Location: " & intErrorLocation.ToString())
        End Try
    End Sub

    Private Sub EditContactK_KeyDown(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        If e.KeyCode = Keys.Escape Then
            Close()
        End If
    End Sub

    Private Sub txtNotes_KeyDown(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles txtNotes.KeyDown
        If e.KeyCode = Keys.Enter Then
            txtNotes.SelectedText = Environment.NewLine
