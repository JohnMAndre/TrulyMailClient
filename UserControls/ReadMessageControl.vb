...
Friend Class ReadMessageControl
    '-- removed form TM6 Private m_strMessageFilename As String = String.Empty
        ...
        Try
            ...
            Select Case m_tm.MessageType
                Case MessageType.Communication, MessageType.Draft, MessageType.ExternalReply, MessageType.RSSArticle, MessageType.PageChangeNotice, MessageType.NoteToSelf
                    ...
                    lblSent2.Text = lblSent.Text '-- for compact view
                    ...
                    If m_tm.BodyFormat = EmailBodyFormatEnum.PlainText AndAlso m_tm.MessageType <> MessageType.Draft Then
                        '-- PlainText
                        '   There are no remote images in plain text
                        HideRemoteImagePanel()

                        pnlBlockedNotice.Visible = False
                    ...
                    Else
                        ...
                        If m_tm.ShouldBlockRemoteImages() Then
                            Dim intBlockedImages As Integer
                            strAlternateHTML = BlockRemoteImages(strHTML, intBlockedImages)
                            If intBlockedImages = 0 Then
                                '-- There were no remote images
                                HideRemoteImagePanel()
                            Else
                                '-- only use the html with blocked images if images were actually blocked
                                '   this allows more mal-formed html in messages (as long as they do not include remote images)
                                strHTML = strAlternateHTML
                                ShowRemoteImagePane()
                            End If
                        Else
                        ...
                        LoadHTMLIntoBrowser(strHTML)

                        lblBlockText.Text = strBlockedNoticeText
                        Me.ToolTip1.SetToolTip(lblBlockText, strBlockedNoticeText) '-- can be very long, so put on tooltip
                        pnlBlockedNotice.Visible = (strBlockedNoticeText.Length > 0) AndAlso (Not AppSettings.DoNotShowReadMessageBlockNotice)
                    End If
    ...
    Private Sub HideRemoteImagePanel()
        pnlRemoteImages.Visible = False
        pnlMain.Dock = DockStyle.Fill
    End Sub
    Private Sub ShowRemoteImagePane()
        pbShowRemoteImageURLs.Enabled = False '-- will show on browser_documentcomplete
        pnlRemoteImages.Visible = True
    End Sub
    Private Sub LoadTextAsBody(ByVal text As String)
        ...
        If m_boolInFullScreenMode Then
            ...
            If m_frmFullScreen.LastReaderControl Is Me.WebBrowser1 Then
                Me.WebBrowser1.Parent = m_pnlReaderParentBeforeFullScreen
                m_frmFullScreen.LoadRTB(Me.rtbBody, m_tm.Filename)
            End If
            m_frmFullScreen.LastFilename = m_tm.Filename
        End If
    End Sub
    Private Sub LoadHTMLIntoBrowser(ByVal html As String)
        Try
            rtbBody.Hide()

            'Log("Loading HTML into browser")
            WebBrowser1.DocumentText = html

            'Log("Loaded HTML into browser")

            Do Until WebBrowser1.ReadyState = WebBrowserReadyState.Complete
                Application.DoEvents()
            Loop

            'Log("Browser ready state set")

            WebBrowser1.Document.Body.SetAttribute("contentEditable", "false")

            'Log("Browser contentEditable set")


            If m_boolInFullScreenMode Then
                Application.DoEvents()
                If m_frmFullScreen.LastReaderControl Is Me.rtbBody Then
                    Me.rtbBody.Parent = m_pnlReaderParentBeforeFullScreen
                    m_frmFullScreen.LoadHTML(Me.WebBrowser1, m_tm.Filename)
                End If
                m_frmFullScreen.LastFilename = m_tm.Filename
            End If

            'WebBrowser1.Document.AttachEventHandler("ondblclick", AddressOf WebBrowserDocument_DoubleClick)

        Catch ex As Exception
            Log(ex)
            If Not ex.Message.StartsWith("Cannot access a disposed object.") Then
                ShowMessageBoxError("There was an error loading the current message. If this error repeats, please contact TrulyMail Support.")
            End If
        End Try
    End Sub
    ...
    Private Sub LoadRecipientList(ByVal xListRecipients As Xml.XmlNodeList, ByVal xListAddresses As Xml.XmlNodeList)
        ...
        'If Not boolAtLeaseOneTrulyMail Then
        '    RaiseEvent EnableUnSendMessage(False)
        'End If
    ...
    Private Sub NotifyIfEmailSentFromTrulyMail()
        ...
        End If
    End Sub
    Private Sub UnBlockRemoteImageForThisItem()
        Try
            WebBrowser1.DocumentText = UnBlockRemoteImages(WebBrowser1.Document.Body.OuterHtml, True, m_tm.Filename)
            If m_tm.MessageType = MessageType.RSSArticle Then
                LoadMessageDelayed()
            Else
                Do Until WebBrowser1.ReadyState = WebBrowserReadyState.Complete OrElse WebBrowser1.ReadyState = WebBrowserReadyState.Interactive
                    Application.DoEvents()
                Loop
            End If
            HideRemoteImagePanel()
        Catch ex As Exception
            Log(ex)
            ShowMessageBoxError("There was an error unblocking remote images for this item." & _
                                Environment.NewLine & Environment.NewLine & _
                                "If this message repeats, please contact TrulyMail Support.")
        End Try
    End Sub
    Private Sub lblFrom_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles llblFrom.LinkClicked, llblFrom2.LinkClicked
    ...
    Private Sub WebBrowser1_DocumentCompleted(ByVal sender As System.Object, ByVal e As System.Windows.Forms.WebBrowserDocumentCompletedEventArgs) Handles WebBrowser1.DocumentCompleted
        pbUnblockRemoteImages.Enabled = True
        pbShowRemoteImageURLs.Enabled = True
    End Sub
...
#End Region
    ...
    Friend Sub PrintMessage()
        ...
        Try
            ...
            If Me.rtbBody.Visible Then
                ...
                Next
            Else
                WebBrowser2.Document.Write(Me.WebBrowser1.DocumentText)
            End If
            strErrorLocation = "s"
    ...
    Private Sub WebBrowser1_Navigating(ByVal sender As System.Object, ByVal e As System.Windows.Forms.WebBrowserNavigatingEventArgs) Handles WebBrowser1.Navigating
        Try
            ...
            ElseIf e.Url.ToString.ToLower.StartsWith("mailto:") Then
                Dim frm As IComposeForm
                If UseFallbackModeNow() Then
                    frm = New NewMessageSimple(e.Url.ToString(), True)
                Else
                    frm = New NewMessage(e.Url.ToString(), True)
                End If
                CType(frm, Form).Show()
    ...
    Public Sub ShowBodyFullScreen()
        Try
            ...
            If rtbBody.Visible Then
                ...
                m_frmFullScreen.LoadRTB(Me.rtbBody, m_tm.Filename)
                'Using frm As New BrowserFullScreenForm(Me.rtbBody, txtSubject.Text, m_strMessageFilename)
            ...
            Else
                '-- Use browser
                m_pnlReaderParentBeforeFullScreen = Me.WebBrowser1.Parent
                If m_frmFullScreen Is Nothing Then
                    m_frmFullScreen = New BrowserFullScreenForm()
                End If

                If m_frmFullScreen.LastReaderControl Is rtbBody Then
                    Me.rtbBody.Parent = m_pnlReaderParentBeforeFullScreen
                End If

                m_frmFullScreen.LoadHTML(Me.WebBrowser1, m_tm.Filename)
                'Using frm As New BrowserFullScreenForm(Me.WebBrowser1, txtSubject.Text, m_strMessageFilename)
                m_boolInFullScreenMode = True
                m_frmFullScreen.ShowDialog(Me)
                m_boolInFullScreenMode = False
                Me.WebBrowser1.Parent = m_pnlReaderParentBeforeFullScreen
    ...
    Private Sub RichTextBox1_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkClickedEventArgs) Handles rtbBody.LinkClicked
        'System.Diagnostics.Process.Start(e.LinkText)
        System.Diagnostics.Process.Start("explorer.exe", e.LinkText) '-- Modified 28 Jan 2017 after realizing that the old way could allow running the link with elevated right
    End Sub
    ...
    Private Sub AddToAddressBookToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles AddToAddressBookToolStripMenuItem.Click
        ...
        ElseIf ctxmnu.SourceControl Is llblFrom2 Then
            addressBookEntry.FullName = m_tm.Sender.FullName
            addressBookEntry.Address = m_tm.Sender.Address
        ElseIf ctxmnu.SourceControl Is llblReplyTo Then
    ...
    Friend Sub ReadMessageAloud()
        ...
        Else
            ...
            If strToRead.Length = 0 Then
                ...
                strToRead = StripHTML(WebBrowser1.DocumentText, True)
            End If
        End If
    ...
    Private Function GetBodyText() As String
        If rtbBody.Visible Then
            Return rtbBody.Text
        Else
            Return StripHTML(WebBrowser1.DocumentText, True)
        End If
    End Function
    Private Function GetSelectedBrowserText() As String
        Try
            Dim htmldoc As mshtml.IHTMLDocument2 = CType(WebBrowser1.Document.DomDocument, mshtml.IHTMLDocument2)
    ...
    Private Sub Zoom(ByVal zoomLevel As Byte)
        Try
            Const OLECMDID_ZOOM As Integer = 63
            ...
            Try
                Select Case zoomLevel
                    Case 1
                        dblValue = 0.5
                        o.ExecWB(OLECMDID_ZOOM, OLECMDEXECOPT_DONTPROMPTUSER, 50, IntPtr.Zero)
                    Case 2
                        dblValue = 0.75
                        o.ExecWB(OLECMDID_ZOOM, OLECMDEXECOPT_DONTPROMPTUSER, 75, IntPtr.Zero)
                    Case 3
                        dblValue = 1
                        o.ExecWB(OLECMDID_ZOOM, OLECMDEXECOPT_DONTPROMPTUSER, 100, IntPtr.Zero)
                    Case 4
                        dblValue = 1.25
                        o.ExecWB(OLECMDID_ZOOM, OLECMDEXECOPT_DONTPROMPTUSER, 125, IntPtr.Zero)
                    Case 5
                        dblValue = 1.5
                        o.ExecWB(OLECMDID_ZOOM, OLECMDEXECOPT_DONTPROMPTUSER, 150, IntPtr.Zero)
                    Case 6
                        dblValue = 1.75
                        o.ExecWB(OLECMDID_ZOOM, OLECMDEXECOPT_DONTPROMPTUSER, 175, IntPtr.Zero)
                    Case 7
                        dblValue = 2
                        o.ExecWB(OLECMDID_ZOOM, OLECMDEXECOPT_DONTPROMPTUSER, 200, IntPtr.Zero)
                    Case 8
                        dblValue = 2.5
                        o.ExecWB(OLECMDID_ZOOM, OLECMDEXECOPT_DONTPROMPTUSER, 250, IntPtr.Zero)
                    Case 9
                        dblValue = 3
                        o.ExecWB(OLECMDID_ZOOM, OLECMDEXECOPT_DONTPROMPTUSER, 300, IntPtr.Zero)
                    Case 10
                        dblValue = 3.5
                        o.ExecWB(OLECMDID_ZOOM, OLECMDEXECOPT_DONTPROMPTUSER, 350, IntPtr.Zero)
                    Case Else
                        dblValue = 4
                        o.ExecWB(OLECMDID_ZOOM, OLECMDEXECOPT_DONTPROMPTUSER, 400, IntPtr.Zero)
                End Select
            Catch ex As Exception
                '-- ignore since we throw the error on InitializeComponents
    ...
    Private Function SelectedAttachmentIsAudio() As String
        ...
        End If
    End Function
    Private Sub olvAttachments_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles olvAttachments.SelectedIndexChanged
        Try
            Dim strFilename As String = SelectedAttachmentIsAudio()
            If strFilename.Length > 0 Then
                pnlAudioPlayer.Visible = True
                Me.AudioPlayer1.LoadMedia(strFilename)
            Else
                pnlAudioPlayer.Visible = False
            End If
        Catch ex As Exception
    ...
    Private Sub picHeaderAreaCollapse_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles picHeaderAreaCollapse.Click
        AppSettings.ReadMessageHeaderCompactMode = True
        SyncCompactLabels()
    End Sub
    Private Sub picHeaderAreaExpand_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles picHeaderAreaExpand.Click
        AppSettings.ReadMessageHeaderCompactMode = False
        SyncCompactLabels()
    End Sub
    Private Sub SyncCompactLabels()
        llblFrom2.Text = llblFrom.Text
        llblFrom2.BackColor = llblFrom.BackColor
        llblFrom2.LinkArea = llblFrom.LinkArea
        ToolTip1.SetToolTip(llblFrom2, ToolTip1.GetToolTip(llblFrom))

        txtSubject2.Text = txtSubject.Text
        txtSubject2.BackColor = SystemColors.Control
        txtSubject.BackColor = SystemColors.Control

        If m_tm.MessageType = MessageType.RSSArticle OrElse m_tm.MessageType = MessageType.PageChangeNotice Then
            If m_tm.Attachments.Count > 0 Then
                SplitContainerRecipAttachments.Panel1Collapsed = True
                SplitContainer2.Panel1Collapsed = False
            Else
                SplitContainer2.Panel1Collapsed = True
            End If
            pnlRSSLink.Visible = True
        Else
            pnlRSSLink.Visible = False
            If AppSettings.ReadMessageHeaderCompactMode Then
                pnlHeaderAreaCompact.Show()
                pnlHeaderAreaFull.Hide()
                pnlBrowserZoom.Hide()
                If olvAttachments.Items.Count > 0 Then
                    '-- show only attachments OLV
                    SplitContainerRecipAttachments.Panel1Collapsed = True
                    SplitContainer2.Panel1Collapsed = False
                Else
                    '-- hide whole split container
                    SplitContainer2.Panel1Collapsed = True
                End If
            Else
                pnlHeaderAreaFull.Show()
                pnlHeaderAreaCompact.Hide()
                If AppSettings.ShowReadingZoomBar Then
                    pnlBrowserZoom.Show()
                Else
                    pnlBrowserZoom.Hide()
                End If
                SplitContainerRecipAttachments.Panel1Collapsed = False
                SplitContainer2.Panel1Collapsed = False
            End If
        End If
    End Sub
    ...
    Private Sub btnFindNext_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnFindNext.Click
        ...
        Else
            RemoveHighlight()
            Static intColorCounter As Integer = 0
            intColorCounter += 1
            Dim strColor As String
            Select Case intColorCounter
                Case 1
                    strColor = "LightBlue"
                Case 2
                    strColor = "LightGreen"
                ...
                Case Else
                    ...
                    intColorCounter = 0
            End Select
            Highlight(txtFindText.Text, strColor)
        End If
    End Sub
    Private Function GetBrowserDocument() As mshtml.IHTMLDocument2
        Return CType(WebBrowser1.Document.DomDocument, mshtml.IHTMLDocument2)
    End Function
    Private Function GetBrowserBodyAsElement() As mshtml.IHTMLBodyElement
        Dim body As mshtml.IHTMLBodyElement = GetBrowserDocument.body
        Return body
    End Function

    Private m_HighlightText As New ArrayList
    '================================================= ===========================================
    Friend Sub Highlight(ByVal TextToHighlight As String, Optional ByVal ExactMatchColor As String = "Yellow", Optional ByVal WordMatchColors As String = "lime, aqua, violet, aquamarine, paleturquoise, gold, lavender, wheat, lightgrey, tomato")
        ...
        If TextToHighlight.Trim <> "" Then
            ...
            Dim body As mshtml.IHTMLBodyElement = GetBrowserBodyAsElement()
            Dim TextRange As mshtml.IHTMLTxtRange = body.createTextRange()
    ...
    Friend Sub HighlightSelectedText(ByVal HighlightAll As Boolean, Optional ByVal ExactMatchColor As String = "yellow")
        Dim TextRange As mshtml.IHTMLTxtRange = GetBrowserDocument.selection.createRange()
    ...
    Private Sub picCloseBlockPanel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles picCloseBlockPanel.Click
        If chkDoNotShowBlockPanel.Checked Then
            AppSettings.DoNotShowReadMessageBlockNotice = True
        End If
        pnlBlockedNotice.Hide()
    End Sub

    Private Sub llblBlockedScriptsInfo_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles llblBlockedScriptsInfo.LinkClicked
        ShowMessageBox("When senders include scripts and IFrames in messages, they may be trying to infect your computer." & _
                       Environment.NewLine & Environment.NewLine & _
                       "To be safe, TrulyMail blocks all scripts and IFrames.", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1)
    End Sub
    ...
    Private Sub pbRemoteImageInfo_Click(sender As System.Object, e As System.EventArgs) Handles pbRemoteImageInfo.Click
    ...
    Private Sub pbShowRemoteImageURLs_Click(sender As System.Object, e As System.EventArgs) Handles pbShowRemoteImageURLs.Click
        Try
            Using frm As New RemoteImagesSelector(WebBrowser1.Document.Body.OuterHtml)
                frm.Text = "Remote Images for: " & txtSubject.Text
                If frm.ShowDialog() = Windows.Forms.DialogResult.OK Then
                    '-- Save the modifications back as the body in the file
                    m_tm.Body = UnBlockRemoteImages(frm.HTML, False, String.Empty)
                    m_tm.Save()

                    '-- load the new body into this message
                    LoadHTMLIntoBrowser(frm.HTML)

                    If Not frm.ImagesRemain Then
                        HideRemoteImagePanel()
                    End If
                End If
            End Using
        Catch ex As Exception
            Log(ex)
            ShowMessageBoxError("There was an error showing the image addresses. If this problem repeats, please contact TrulyMail Support.")
        End Try
    End Sub

    Private Sub pbUnblockRemoteImages_Click(sender As System.Object, e As System.EventArgs) Handles pbUnblockRemoteImages.Click
        UnBlockRemoteImageForThisItem()
    End Sub

    Private Sub pbNeverBlockRemoteImagesForSender_Click(sender As System.Object, e As System.EventArgs) Handles pbNeverBlockRemoteImagesForSender.Click
        Try
            If m_tm.MessageType = MessageType.RSSArticle Then
                Dim strSender As String = m_tm.SenderName.ToLower()
                If Not AppSettings.RSSSenderAllowRemoteImages.ContainsKey(strSender) Then
                    AppSettings.RSSSenderAllowRemoteImages.Add(strSender, Nothing)
                End If
                LoadMessageDelayed()
            Else
                Dim entry As AddressBookEntry = ABook.GetContactByAddress(m_tm.Sender.Address)
                If entry Is Nothing Then
                    ...
                    entry.Address = m_tm.Sender.Address
                    If m_tm.Sender.FullName.Length = 0 Then
                        entry.FullName = GetUserNameFromEmailAddress(m_tm.Sender.Address)
                    Else
                        entry.FullName = m_tm.Sender.FullName
                    End If
                    entry.RemoteImages = True

                    ABook.Add(entry)
                    ShowMessageBox("The sender was added to your address book and set to allow remote images.", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1)
                Else
                    entry.Active = True
                    entry.RemoteImages = True
                End If
                ABook.Save()
                Try
                    '-- if loading fails, then we just call load msg delayed
                    WebBrowser1.DocumentText = UnBlockRemoteImages(WebBrowser1.Document.Body.OuterHtml, False, m_tm.Filename)
                Catch ex As Exception
                    Log(ex)
                    LoadMessageDelayed()
                End Try
        ...
        Catch ex As Exception
            Log(ex)
            ShowMessageBoxError("There was an error adding to the 'never block remote images' list. Please contact TrulyMail Support.")
        End Try
    End Sub
    ...
    Public Sub NothingToDisplay()
        If m_frmFullScreen IsNot Nothing Then
            If m_frmFullScreen.LastReaderControl Is WebBrowser1 Then
                Me.WebBrowser1.Parent = m_pnlReaderParentBeforeFullScreen
                Me.WebBrowser1.BringToFront()
            Else
                Me.rtbBody.Parent = m_pnlReaderParentBeforeFullScreen
        ...
        End If
    End Sub
    Public Sub ConvertCurrentMessageToPlainText()
        Try
            If rtbBody.Visible Then
                Application.DoEvents()
                '-- nothing to do, it's already text
            Else
                Application.DoEvents()
                Dim strText As String = StripHTML(WebBrowser1.DocumentText)
                LoadTextAsBody(strText)
            End If
        Catch ex As Exception
            Log(ex)
            ShowMessageBoxError("There was an error converting the message to plain text." & CONTACT_TRULYMAIL_IFPROBLEMSPERSIST_MESSAGE)
        End Try
    End Sub

    'Private Sub WebBrowserDocument_DoubleClick(sender As Object, e As EventArgs)
    '    Try
    '        Dim doc As mshtml.IHTMLDocument = CType(WebBrowser1.Document.DomDocument, mshtml.IHTMLDocument)
    '        Dim sel As mshtml.IHTMLSelectionObject = CType(doc.selection, mshtml.IHTMLSelectionObject)
    '        Dim rng As mshtml.IHTMLTxtRange = CType(sel.createRange(), mshtml.IHTMLTxtRange)
    '        'rng.execCommand("IDM_SELECTALL", False)
    '        rng.expand("word")
    '        rng.select()
    '        rng.pasteHTML("blah blah blah")
    '        Application.DoEvents()
    '    Catch ex As Exception
    '        Application.DoEvents()
    '    End Try
    ...
    Private Sub NewEmailToThisContactToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles NewEmailToThisContactToolStripMenuItem.Click
        ...
        Try
            ...
            If strAddress IsNot Nothing Then
                intErrorLocation = 10
                Dim msg As NewMessage = MainFormReference.NewMessage()
